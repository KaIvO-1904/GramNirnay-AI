    ...
    def run_pipeline(self, user_input: str, profile: Optional[Any] = None, financial_params: Optional[Any] = None) -> PipelineState:
        ...
        try:
            ...
            if profile and financial_params:
                # Use provided data instead of LLM interpretation
            ...
            # 3. CALCULATION
            # Use the existing FinancialEngine.compute_full_model
            # Note: FinancialEngine currently uses a dict, so we convert.
            fin_data = self.financial_engine.compute_full_model(state.financial_params.model_dump())

            # Map dict to Pydantic model for the rest of the pipeline
            ...
            # 5. UPGRADED VIABILITY AGGREGATION
            ...
            # Handle known architecture errors
